...
- **M14 · Factibilidad/planes: las "atrasadas" necesitan PISO de fecha, no solo techo.** 11-jun · Sebastián: "factibilidad muestra producciones antiguas, no una realidad". `plan_factibilidad` incluía toda producción con `fecha < hoy AND estado pendiente AND inventario_descontado_at IS NULL` SIN límite hacia atrás → arrastraba lotes programados hace meses, nunca hechos ni cancelados (zombies). Regla: al incluir atrasadas/pendientes pasadas, acotá a una ventana reciente (default 30d · `fecha >= hoy - atraso_max`). Un lote vencido hace >30d sin ejecutar casi nunca es demanda real. ⚠ El fondo es data sucia: `produccion_programada` acumula filas viejas 'pendiente' que nadie cierra · conviene un limpiador/cron que las marque (no se borran lo Fijo a ciegas · ver M3 y reglas de Fijo).
