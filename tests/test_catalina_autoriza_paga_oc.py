...
OC_AUTORIZA_USERS={'catalina'} la exceptúa; Mayra (contadora pura) sigue bloqueada.
"""
import os
import sqlite3
...
def test_mayra_sigue_bloqueada_sod(app, db_clean):
    """Mayra (contadora pura) NO está en OC_AUTORIZA_USERS → sigue bloqueada (SoD)."""
    _seed_oc('OC-CAT-2')
    c = _login(app, 'mayra')
    r = c.patch('/api/ordenes-compra/OC-CAT-2/autorizar', json={}, headers=csrf_headers())
    assert r.status_code == 403 and (r.get_json() or {}).get('codigo') == 'SEGREGATION_OF_DUTIES', \
        f"Mayra (contadora) debe seguir bloqueada por SoD · {r.status_code} {r.data[:200]}"
