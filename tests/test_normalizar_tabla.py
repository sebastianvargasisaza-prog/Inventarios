...
def _limpiar(app):
    ...
    with app.app_context():
        ...
        c.execute("DELETE FROM maestro_mee WHERE codigo LIKE 'NT2-%'")
        c.commit()
...
def _pres(app, producto=PROD, frasco='NT2-FR', **kw):
    from database import get_db
    with app.app_context():
        c = get_db()
...
def test_ARRASTRA_por_nombre(app, admin_client):
    ...
    _limpiar(app)
    _mee(app, 'NT2-FR', 'FRASCO VIDRIO 30', 'Frasco')
    _mee(app, 'NT2-ETQ', 'ETIQUETA ZANAHORIA 30', 'Etiqueta')
    pid = _pres(app)
    f, _ = _fila(admin_client, pid)
    assert f and f['sugerido'].get('etiqueta') == 'NT2-ETQ', \
...
def test_DEJA_VACIO_lo_que_no_coincide(app, admin_client):
    ...
    _limpiar(app)
    _mee(app, 'NT2-FR', 'FRASCO VIDRIO 30', 'Frasco')
    _mee(app, 'NT2-ETQ', 'ETIQUETA MANZANILLA 30', 'Etiqueta')     # nada que ver
    pid = _pres(app)
    f, _ = _fila(admin_client, pid)
...
def test_si_DOS_empatan_deja_VACIO_y_lo_DICE(app, admin_client):
    ...
    _limpiar(app)
    _mee(app, 'NT2-FR', 'FRASCO VIDRIO 30', 'Frasco')
    _mee(app, 'NT2-ETQ1', 'ETIQUETA ZANAHORIA FRENTE', 'Etiqueta')
    _mee(app, 'NT2-ETQ2', 'ETIQUETA ZANAHORIA DORSO', 'Etiqueta')
    pid = _pres(app)
    f, _ = _fila(admin_client, pid)
...
def test_NO_propone_encima_de_lo_que_YA_esta(app, admin_client):
    ...
    _mee(app, 'NT2-FR', 'FRASCO VIDRIO 30', 'Frasco')
    _mee(app, 'NT2-ETQ', 'ETIQUETA ZANAHORIA 30', 'Etiqueta')
    _mee(app, 'NT2-OTRA', 'ETIQUETA VIEJA', 'Etiqueta')
    pid = _pres(app, etq='NT2-OTRA')
    f, _ = _fila(admin_client, pid)
    assert f['actual']['etiqueta'] == 'NT2-OTRA'
...
def test_lo_que_NO_se_pudo_guardar_se_DICE(app, admin_client):
    ...
    _limpiar(app)
    _mee(app, 'NT2-FR', 'FRASCO VIDRIO 30', 'Frasco')
    _mee(app, 'NT2-TAP', 'TAPA NEGRA 30', 'Tapa')
    pid = _pres(app)
    r = admin_client.post('/api/mee/normalizar-guardar',
                          json={'filas': [{'id': pid, 'tapa': 'NT2-TAP', 'caja': 'FANTASMA'}]},
                          headers={'Origin': 'http://localhost'})
    j = r.get_json()
...
def test_cada_columna_ofrece_SU_categoria(app, admin_client):
    ...
    assert 'NT2-TAP' in tapas, 'la tapa no aparece en su columna'
    assert 'NT2-TAP' not in cajas, 'la tapa aparece en la columna de la caja'
    _limpiar(app)
