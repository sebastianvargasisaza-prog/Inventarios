...
def simular_produccion():
    ...
    def _es_infinita(mid, nom, cs):
        # AGUA (desionizada/purificada/destilada) = fabricación propia · infinita · NUNCA faltante.
        # Se detecta por el flag controla_stock=0 Y por NOMBRE/CÓDIGO (hay códigos de agua fantasma
        # MPAGUAL01/MPAGUALI02 sin el flag que inflaban el faltante · Sebastián 9-jul).
        if int(cs or 0) == 0:
            return True
...
def _rotulo_logo_src(c):
    ...
    try:
        r = c.execute("SELECT valor FROM app_settings WHERE clave='logo_espagiria'").fetchone()
        return ((r[0] if r else '') or '').strip() or '/static/logos/espagiria.svg'
    except Exception:
        return '/static/logos/espagiria.svg'


def _rotulo_recep_css(lw, lh):
...
def importar_conteo_page():
    ...
    html = (
        ...
        'window.verCat=function(cat){document.querySelectorAll("#tabla tbody tr").forEach(function(tr){var c=tr.getAttribute("data-cat");var show=(cat==="todas")||(cat==="mapean"&&c==="saldo")||(cat==="nomapean"&&c==="problema");tr.style.display=show?"":"none";});};'
