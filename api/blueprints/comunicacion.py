...
def _email_tarea_asignada(tarea_id, usuarios, rol, asignador):
    ...
    try:
        ...
        body = (
            ...
            f'<div style="background:var(--cx-bg-alt, #fafaf9);border:1px solid var(--cx-border, #e7e5e4);border-radius:8px;padding:20px;">'
            ...
            '</div>'
            f'<h2 style="color:var(--cx-text, #1c1917);margin:8px 0 12px;">{td["titulo"]}</h2>'
            f'<p style="color:var(--cx-text-soft, #57534e);line-height:1.6;font-size:14px;">{td.get("descripcion","") or ""}</p>'
            '<table style="font-size:13px;color:var(--cx-text-soft, #44403c);margin:14px 0;">'
            f'<tr><td style="padding:4px 12px 4px 0;color:var(--cx-text-faint, #a8a29e);">Prioridad:</td>'
            f'<td><strong style="color:{prio_color}">{td.get("prioridad", "Media")}</strong></td></tr>'
            f'<tr><td style="padding:4px 12px 4px 0;color:var(--cx-text-faint, #a8a29e);">Área:</td>'
            f'<td>{td.get("area","-") or "-"}</td></tr>'
            f'<tr><td style="padding:4px 12px 4px 0;color:var(--cx-text-faint, #a8a29e);">Comprometida:</td>'
            f'<td>{td.get("fecha_compromiso","sin fecha") or "sin fecha"}</td></tr>'
            f'<tr><td style="padding:4px 12px 4px 0;color:var(--cx-text-faint, #a8a29e);">Asignó:</td>'
            f'<td>{asignador or "sistema"}</td></tr>'
            '</table>'
            '<p style="margin-top:18px;">'
            f'<a href="{APP_BASE_URL}/comunicacion" '
            'style="background:var(--cx-text, #1c1917);color:#fff;padding:10px 20px;border-radius:6px;'
            'text-decoration:none;font-weight:600;display:inline-block;">'
            'Ver tarea →</a>'
            '</p>'
            '<p style="color:var(--cx-text-faint, #a8a29e);font-size:11px;margin-top:24px;">'
