...
def rrhh_comprobante(periodo, eid):
    ...
    if estado == "Pagada":
        badge = "background:var(--cx-success, #166534);color:#fff"
    elif estado == "Aprobada":
        badge = "background:var(--cx-success-pale, #dcfce7);color:var(--cx-success-text, #166534)"
    else:
        badge = "background:var(--cx-warn-pale, #fef3c7);color:var(--cx-warn-text, #92400e)"
    ...
    html = (
        ...
        "<style>body{font-family:Arial,sans-serif;font-size:12px;margin:20px;color:var(--cx-text, #1C1917);}"
        ".hdr{text-align:center;border-bottom:2px solid var(--cx-text, #1C1917);padding-bottom:8px;margin-bottom:12px;}"
        ".hdr h2{margin:0;font-size:15px;}.hdr p{margin:2px;font-size:11px;color:var(--cx-text-soft, #555);}"
        "table{width:100%;border-collapse:collapse;margin-bottom:8px;}"
        "td{padding:4px 8px;}tr:nth-child(even){background:#F9F8F7;}"
        ".lbl{color:var(--cx-text-mute, #666);}.val{text-align:right;font-weight:600;}"
        ".sec{background:var(--cx-text, #1C1917);color:#fff;padding:3px 8px;font-weight:700;font-size:11px;margin-top:8px;}"
        ".neto{font-size:18px;font-weight:700;color:var(--cx-primary-text, #6D28D9);text-align:right;padding:8px;border-top:2px solid var(--cx-primary, #6D28D9);}"
        ".footer{font-size:10px;color:var(--cx-text-faint, #999);text-align:center;margin-top:16px;border-top:1px solid var(--cx-border, #ddd);padding-top:6px;}"
        ...
        "<p><strong>HHA Group - Laboratorio Espagiria / &Aacute;NIMUS Lab</strong></p>"
        ...
        +(("<div class='sec' style='margin-top:12px;background:var(--cx-success, #15803d)'>DATOS BANCARIOS PARA PAGO</div><table>"
        "<tr><td class='lbl'>Banco:</td><td><strong>"+(banco or "Sin registrar")+"</strong></td>"
        "<td class='lbl'>Tipo de cuenta:</td><td>"+(tipo_cta or "—")+"</td></tr>"
        "<tr><td class='lbl'>N&uacute;mero de cuenta:</td><td colspan='3' style='font-family:monospace;font-weight:700;color:var(--cx-success-text, #166534);font-size:13px;'>"+(num_cta or "—")+"</td></tr>"
        ...
        "<td class='lbl'>Costo total empresa</td><td class='val' style='color:var(--cx-primary-text, #6D28D9)'>"+cop(neto+ap_tot)+"</td></tr>"
        "</table>"
        "<div class='footer'>Generado por Sistema HHA Group &nbsp;|&nbsp; "+periodo+"</div>"
        "<div style='text-align:center;margin-top:10px'>"
        "<button onclick='window.print()' style='padding:8px 20px;background:var(--cx-text, #1C1917);color:#fff;border:none;border-radius:6px;cursor:pointer'>&#128424; Imprimir</button>"
        "</div></body></html>"
    )
