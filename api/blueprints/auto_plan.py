...
# Mayerlin reporta. App ajusta automáticamente la cadencia siguiente".

@bp.route('/api/planta/registrar-lote-real', methods=['POST'])
def registrar_lote_real():
    """Registra un lote efectivamente producido vs lo planeado.

    Body:
      producto: nombre exacto del SKU (ej. 'SUERO HIDRATANTE AH 1.5%')
      kg_real: cantidad realmente producida en kg
      fecha: opcional (default hoy)
      lote: opcional (lote_produccion)
      operador: opcional (default sesión)
      observaciones: opcional
      descontar_mp: bool (default true) - descuenta MP de movimientos según fórmula

    Side effects:
      1. Inserta en producciones (estado 'completado')
      2. Si descontar_mp: por cada item de fórmula descuenta cantidad proporcional
      3. Loggea en audit_log
    """
    if 'compras_user' not in session:
        return jsonify({'error': 'No autorizado'}), 401
    user = session.get('compras_user', '')
    ...
    producto = (d.get('producto') or '').strip()
    try:
        kg_real = float(d.get('kg_real') or 0)
    except Exception:
        kg_real = 0
    if not producto or kg_real <= 0:
        return jsonify({'error': 'producto y kg_real (>0) requeridos'}), 400

    fecha = (d.get('fecha') or datetime.now().date().isoformat()).strip()
    lote_id = (d.get('lote') or '').strip()
    operador = (d.get('operador') or user or 'sin-operador').strip()
    observ = (d.get('observaciones') or '').strip()
    descontar_mp = d.get('descontar_mp', True)
    ...
    try:
        c.execute("""
            INSERT INTO producciones (producto, cantidad, fecha, estado, observaciones, operador, lote)
            VALUES (?, ?, ?, 'completado', ?, ?, ?)
        """, (producto, kg_real, fecha, observ, operador, lote_id))
        prod_id = c.lastrowid
    except Exception as e:
        return jsonify({'error': f'Error guardando producción: {e}'}), 500

    # 2. Descontar MP según fórmula (si aplica)
    mp_descontada = []
    mp_faltante = []
    if descontar_mp:
        items = c.execute("""
        ...
        for mat_id, mat_nom, pct in items:
            ...
            stock_g = float(stock_row[0] if stock_row else 0)
            if stock_g < req_g:
                mp_faltante.append({'material_id': mat_id, 'material_nombre': mat_nom, 'requerido_g': req_g, 'stock_g': stock_g})
                # Igual descontar lo que hay (queda negativo)
            try:
                c.execute("""
                    INSERT INTO movimientos (material_id, material_nombre, cantidad, tipo, observaciones, fecha)
                    VALUES (?, ?, ?, 'Salida', ?, ?)
                """, (mat_id, mat_nom or mat_id, req_g,
                      f'Producción #{prod_id} · {producto} ({kg_real}kg) · operador {operador}',
                      datetime.now().isoformat()))
                mp_descontada.append({'material_id': mat_id, 'material_nombre': mat_nom, 'descontado_g': round(req_g, 1)})
            except Exception as e:
                log.warning(f'No descontó MP {mat_id}: {e}')

    # 3. Audit log
    try:
        c.execute("""
            INSERT INTO audit_log (usuario, accion, tabla, registro_id, detalle, ip, fecha)
            VALUES (?, 'PRODUCCION_REAL', 'producciones', ?, ?, '', ?)
        """, (user, str(prod_id),
              f'Producto: {producto} · kg: {kg_real} · operador: {operador} · MP descontadas: {len(mp_descontada)} · MP faltantes: {len(mp_faltante)}',
              datetime.now().isoformat()))
    except Exception:
        pass
    ...
    return jsonify({
        ...
        'kg_real': kg_real,
        'fecha': fecha,
        'mp_descontada': mp_descontada,
        'mp_faltante': mp_faltante,
        'aviso_mp_faltante': len(mp_faltante) > 0,
        'mensaje': f'✅ Lote registrado · {len(mp_descontada)} MPs descontadas' + (f' · ⚠️ {len(mp_faltante)} MPs quedaron en stock negativo' if mp_faltante else ''),
    })


@bp.route('/api/planta/producciones-recientes', methods=['GET'])
def producciones_recientes():
    """Lista las últimas N producciones registradas (para el dashboard)."""
    if 'compras_user' not in session:
        return jsonify({'error': 'No autorizado'}), 401
    n = int(request.args.get('n') or 20)
    conn = get_db(); c = conn.cursor()
    rows = c.execute("""
        SELECT id, producto, cantidad, fecha, estado, operador, lote, observaciones
        FROM producciones
    ...
    return jsonify({
        'producciones': [
            {'id': r[0], 'producto': r[1], 'kg': r[2], 'fecha': r[3], 'estado': r[4],
             'operador': r[5] or '', 'lote': r[6] or '', 'observaciones': r[7] or ''}
            for r in rows
        ]
    })
