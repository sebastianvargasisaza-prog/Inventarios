...
MIGRATIONS: list[tuple[int, str, list[str]]] = [
    ...
    (351, "Blush Balm · fórmula v4 (Alejandro · Instructivo v4 15-jul). Etapa 1: corrige la fórmula ACTIVA 'BLUSH BALM' en sitio a la v4 (3 fases · % p/p). CAMBIOS vs la anterior: (a) el slot del 7.5% estaba mal codificado como AGUA DESIONIZADA (imposible en un stick anhidro) → se reemplaza por el PIGMENTO 'Pigmentos CI (mezcla)' MPPIGCI01 al 7.179% (Fase B · CI según tono); (b) Phenyl Trimethicone (MP00127) y Dicaprylyl Carbonate (MP00040) suben 20.271→21.146; (c) PMSS (MP00055) baja 3.0→2.0; (d) Polyglyceryl-2 Triisostearate (MP00051) baja 10.0→9.571. Suma=100%. El PIGMENTO se pone controla_stock=0 (DIFERIDO: los pigmentos aún no están en el sistema · se abrirá por tono en Etapa 2 cuando se carguen · NO se compra por ahora, igual que el agua). Base+blend real (semi-terminados por tono) queda para la Etapa 3 (motor de ensamble). Idempotente (DELETE+INSERT). PG-safe.", [
        ...
        # reintento con esta guarda la restaura). Diagnóstico contra prod (15-jul): MPBNIT01
        # (Boron Nitride) y MP00190 (Palmitoyl Tripeptide-1) estaban AUSENTES del maestro activo
        # (sus hermanos peptídicos MP00172/MP00174 sí están · son ingredientes reales de la v4).
        # Crear-si-falta los que pueden faltar + reactivar los 21 (autoritativos · PDF Alejandro).
        "INSERT INTO maestro_mps (codigo_mp, nombre_comercial, nombre_inci, activo, controla_stock) "
        "SELECT 'MPBNIT01','Boron Nitride','Boron Nitride',1,1 "
        "WHERE NOT EXISTS (SELECT 1 FROM maestro_mps WHERE codigo_mp='MPBNIT01')",
        "INSERT INTO maestro_mps (codigo_mp, nombre_comercial, nombre_inci, activo, controla_stock) "
        "SELECT 'MP00190','Palmitoyl Tripeptide-1','Palmitoyl Tripeptide-1',1,1 "
        "WHERE NOT EXISTS (SELECT 1 FROM maestro_mps WHERE codigo_mp='MP00190')",
        "INSERT INTO maestro_mps (codigo_mp, nombre_comercial, nombre_inci, activo, controla_stock) "
        ...
        # reactivar los 21 códigos de la v4 (idempotente · no-op si ya activos)
        "UPDATE maestro_mps SET activo=1 WHERE codigo_mp IN "
        "('MP00127','MP00040','MP00257','MP00024','MP00041','MPCOCP01','MP00055','MPBNIT01',"
        "'MP00054','MP00077','MP00063','MP00051','MPPIGCI01','MP00207','MP00103','MP00078',"
        "'MP00101','MP00025','MP00190','MP00172','MP00174')",
        # pigmento con compra DIFERIDA (pigmentos aún no cargados por tono · como el agua · M16)
    ...
    (331, "ANIMUSLASH reconstruida desde MyBatch (OP-2026-74 · Sebastian: MyBatch=verdad · 1-jul). MyBatch usa CODIGOS distintos a EOS -> mapeo por INCI: 27 coinciden, Cafeina MyBatch MP00069 -> EOS MP00118, Butylene Glycol (MyBatch MP00258) NO existia en EOS -> se crea MP00298. La formula EOS estaba vieja/distinta -> rebuild completo (delete + 29 MP + agua 87.71%). sin agua 12.29%. Verificado == MyBatch mapeado. nombres desde el catalogo por subquery.", [
        ...
        "DELETE FROM formula_items WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH'",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00169',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00169'),'MP00169'),0.3,ROUND(0.3/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00079',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00079'),'MP00079'),0.1,ROUND(0.1/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00147',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00147'),'MP00147'),0.005,ROUND(0.005/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00178',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00178'),'MP00178'),0.003,ROUND(0.003/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00194',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00194'),'MP00194'),0.05,ROUND(0.05/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00170',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00170'),'MP00170'),0.02,ROUND(0.02/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00164',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00164'),'MP00164'),0.005,ROUND(0.005/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00193',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00193'),'MP00193'),0.006,ROUND(0.006/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00187',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00187'),'MP00187'),0.05,ROUND(0.05/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00168',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00168'),'MP00168'),0.005,ROUND(0.005/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00236',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00236'),'MP00236'),0.2,ROUND(0.2/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00118',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00118'),'MP00118'),1.0,ROUND(1.0/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00285',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00285'),'MP00285'),0.005,ROUND(0.005/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00245',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00245'),'MP00245'),0.5,ROUND(0.5/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00256',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00256'),'MP00256'),1.0,ROUND(1.0/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00298',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00298'),'MP00298'),2.0,ROUND(2.0/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00171',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00171'),'MP00171'),0.02,ROUND(0.02/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00231',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00231'),'MP00231'),0.05,ROUND(0.05/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00151',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00151'),'MP00151'),0.005,ROUND(0.005/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00072',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00072'),'MP00072'),0.5,ROUND(0.5/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00241',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00241'),'MP00241'),0.001,ROUND(0.001/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00215',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00215'),'MP00215'),0.1,ROUND(0.1/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00150',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00150'),'MP00150'),0.05,ROUND(0.05/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00142',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00142'),'MP00142'),0.06,ROUND(0.06/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00068',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00068'),'MP00068'),1.0,ROUND(1.0/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00163',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00163'),'MP00163'),0.2,ROUND(0.2/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00174',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00174'),'MP00174'),0.005,ROUND(0.005/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00046',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00046'),'MP00046'),0.05,ROUND(0.05/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MP00148',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MP00148'),'MP00148'),5.0,ROUND(5.0/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ANIMUSLASH','MPAGUALI01',COALESCE((SELECT nombre_comercial FROM maestro_mps WHERE codigo_mp='MPAGUALI01'),'MPAGUALI01'),87.71,ROUND(87.71/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ANIMUSLASH' LIMIT 1),0)*1000,4)",
    ]),
    ...
    (325, "GEL HIDRATANTE vs MASTER (Sebastian · dump prod 1-jul): (1) prod tiene MP00137 (Argania/Argan) donde el master pide MP00030 (Beauty Sensoft · Propylheptyl caprylate) - material DISTINTO, swap (caso M19, mismo 3.0%). (2) faltaba MP00127 (BM-956 · Phenyl trimethicone) 1.75% - agregar. (3) grado Centella MP00176->MP00181 (plano) 0.03%. (4) sacar MP00245 (1,2-Hexanediol 0.3%) que sobra. Panthenol MP00110~MP00236 se deja. Verificado inyectando estado prod == master.", [
        ...
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'GEL HIDRATANTE','MP00127','BM-956',1.75,ROUND(1.75/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='GEL HIDRATANTE' LIMIT 1),0)*1000,4) WHERE NOT EXISTS (SELECT 1 FROM formula_items WHERE UPPER(TRIM(producto_nombre))='GEL HIDRATANTE' AND UPPER(TRIM(material_id))='MP00127')",
        "DELETE FROM formula_items WHERE UPPER(TRIM(producto_nombre))='GEL HIDRATANTE' AND UPPER(TRIM(material_id))='MP00245'",
    ]),
    (324, "Reconciliacion formulas vs MASTER (Sebastian · dump prod diag-formulas-dump 1-jul): la app diverge del master. (1) GRADO de Centella: prod usa MP00176 (triterpenos 80%) donde el master pide MP00181 (extracto plano) - swap en 8 formulas (mismo %). (2) ESENCIA DE CENTELLA: MP00176 0.15->0.10 + agrega MP00181 0.15 (los 2 grados). (3) N-acetil glucosamina (MP00262) que faltaba: NIACINAMIDA 0.5% + CONTORNO RETINALDEHIDO 1.5%. (4) SUERO EXFOLIANTE BHA: quita MP00187 (Myristoyl pentapeptide-17 equivocado) + MP00250 0.0015->0.002. Panthenol MP00110~MP00236 se deja (mismo material puenteado). Verificado inyectando el estado prod y confirmando == master.", [
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='LIMPIADOR FACIAL BHA 2%' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='LIMPIADOR FACIAL BHA 2%' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='SUERO ANTIOXIDANTE RENOVA C10' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='SUERO ANTIOXIDANTE RENOVA C10' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='SUERO ILUMINADOR TRX' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='SUERO ILUMINADOR TRX' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='SUERO EXFOLIANTE NOVA PHA' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='SUERO EXFOLIANTE NOVA PHA' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='CONTORNO DE CAFEINA' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='CONTORNO DE CAFEINA' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='CONTORNO DE OJOS MULTIPEPTIDOS' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='CONTORNO DE OJOS MULTIPEPTIDOS' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='LIMPIADOR FACIAL HIDRATANTE' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='LIMPIADOR FACIAL HIDRATANTE' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET material_id='MP00181', material_nombre='Centella asiatica extract' WHERE UPPER(TRIM(producto_nombre))='SUERO EXFOLIANTE BHA 2%' AND UPPER(TRIM(material_id))='MP00176' AND NOT EXISTS (SELECT 1 FROM formula_items f2 WHERE UPPER(TRIM(f2.producto_nombre))='SUERO EXFOLIANTE BHA 2%' AND UPPER(TRIM(f2.material_id))='MP00181')",
        "UPDATE formula_items SET porcentaje=0.1, cantidad_g_por_lote=ROUND(0.1/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ESENCIA DE CENTELLA ASIATICA' LIMIT 1),0)*1000,4) WHERE UPPER(TRIM(producto_nombre))='ESENCIA DE CENTELLA ASIATICA' AND UPPER(TRIM(material_id))='MP00176'",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'ESENCIA DE CENTELLA ASIATICA','MP00181','Centella asiatica extract',0.15,ROUND(0.15/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='ESENCIA DE CENTELLA ASIATICA' LIMIT 1),0)*1000,4) WHERE NOT EXISTS (SELECT 1 FROM formula_items WHERE UPPER(TRIM(producto_nombre))='ESENCIA DE CENTELLA ASIATICA' AND UPPER(TRIM(material_id))='MP00181')",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'SUERO DE NIACINAMIDA 5% FORMULA NUEVA','MP00262','N-acetil glucosamina',0.5,ROUND(0.5/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='SUERO DE NIACINAMIDA 5% FORMULA NUEVA' LIMIT 1),0)*1000,4) WHERE NOT EXISTS (SELECT 1 FROM formula_items WHERE UPPER(TRIM(producto_nombre))='SUERO DE NIACINAMIDA 5% FORMULA NUEVA' AND UPPER(TRIM(material_id))='MP00262')",
        "INSERT INTO formula_items (producto_nombre,material_id,material_nombre,porcentaje,cantidad_g_por_lote) SELECT 'CONTORNO DE OJOS RETINALDEHIDO 0.05%','MP00262','N-acetil glucosamina',1.5,ROUND(1.5/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='CONTORNO DE OJOS RETINALDEHIDO 0.05%' LIMIT 1),0)*1000,4) WHERE NOT EXISTS (SELECT 1 FROM formula_items WHERE UPPER(TRIM(producto_nombre))='CONTORNO DE OJOS RETINALDEHIDO 0.05%' AND UPPER(TRIM(material_id))='MP00262')",
        "DELETE FROM formula_items WHERE UPPER(TRIM(producto_nombre))='SUERO EXFOLIANTE BHA 2%' AND UPPER(TRIM(material_id))='MP00187'",
        "UPDATE formula_items SET porcentaje=0.002, cantidad_g_por_lote=ROUND(0.002/100.0*COALESCE((SELECT lote_size_kg FROM formula_headers WHERE UPPER(TRIM(producto_nombre))='SUERO EXFOLIANTE BHA 2%' LIMIT 1),0)*1000,4) WHERE UPPER(TRIM(producto_nombre))='SUERO EXFOLIANTE BHA 2%' AND UPPER(TRIM(material_id))='MP00250'",
    ]),
    ...
    (310, "Envases - mapear Booster + Vitamina C 15/30 (Sebastian 28-jun): Booster->envase que dice BOOSTER (15ml); Vitamina C+ DOS presentaciones -> cuadrado 15ml y cuadrado 30ml. delete-then-insert por volumen (sin doble conteo), envase por descripcion/codigo. EXISTS guard. PG-safe.", [
        "DELETE FROM producto_presentaciones WHERE UPPER(producto_nombre) LIKE '%BOOSTER%' AND presentacion_codigo IN ('V15','PPAL','AUTO-15')",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V15', '15ml', 15, (SELECT codigo FROM maestro_mee WHERE (UPPER(descripcion) LIKE '%BOOSTER%' OR UPPER(codigo) LIKE '%BOOSTER%') AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE UPPER(fh.producto_nombre) LIKE '%BOOSTER%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE (UPPER(descripcion) LIKE '%BOOSTER%' OR UPPER(codigo) LIKE '%BOOSTER%') AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo')",
        "DELETE FROM producto_presentaciones WHERE UPPER(producto_nombre) LIKE '%VITAMINA C%' AND presentacion_codigo IN ('V15','PPAL','AUTO-15')",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V15', '15ml', 15, (SELECT codigo FROM maestro_mee WHERE ((UPPER(codigo) LIKE '%CUADR%' OR UPPER(descripcion) LIKE '%CUADRADO%') AND (UPPER(codigo) LIKE '%15' OR UPPER(descripcion) LIKE '%15ML%')) AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE UPPER(fh.producto_nombre) LIKE '%VITAMINA C%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE ((UPPER(codigo) LIKE '%CUADR%' OR UPPER(descripcion) LIKE '%CUADRADO%') AND (UPPER(codigo) LIKE '%15' OR UPPER(descripcion) LIKE '%15ML%')) AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo')",
        "DELETE FROM producto_presentaciones WHERE UPPER(producto_nombre) LIKE '%VITAMINA C%' AND presentacion_codigo IN ('V30','PPAL','AUTO-30')",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V30', '30ml', 30, (SELECT codigo FROM maestro_mee WHERE ((UPPER(codigo) LIKE '%CUADR%' OR UPPER(descripcion) LIKE '%CUADRADO%') AND (UPPER(codigo) LIKE '%30' OR UPPER(descripcion) LIKE '%30ML%')) AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE UPPER(fh.producto_nombre) LIKE '%VITAMINA C%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE ((UPPER(codigo) LIKE '%CUADR%' OR UPPER(descripcion) LIKE '%CUADRADO%') AND (UPPER(codigo) LIKE '%30' OR UPPER(descripcion) LIKE '%30ML%')) AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo')",
    ]),
    (309, "Envases - crear 3 frascos que faltaban + descontinuar B3+BHA + mapear (Sebastian 28-jun): Blush 6ml aluminio (FR-AL-BLUSH-6, todos los tonos), 10ml vidrio con gotero (FR-VID-GOTERO-10, contorno cafeina + hialuronico), 50ml opalizado (FR-VID-OPALIZADO-50, centella). Emulsion B3+BHA activo=0 (reversible). Idempotente (NOT EXISTS).", [
        "INSERT INTO maestro_mee (codigo, descripcion, categoria, stock_actual, estado, fecha_creacion) SELECT 'FR-AL-BLUSH-6', 'FRASCO ALUMINIO BLUSH BALM 6ml', 'Frasco', 0, 'Activo', '2026-06-28 00:00:00' WHERE NOT EXISTS (SELECT 1 FROM maestro_mee WHERE codigo='FR-AL-BLUSH-6')",
        "INSERT INTO maestro_mee (codigo, descripcion, categoria, stock_actual, estado, fecha_creacion) SELECT 'FR-VID-GOTERO-10', 'FRASCO VIDRIO CON GOTERO 10ml', 'Frasco', 0, 'Activo', '2026-06-28 00:00:00' WHERE NOT EXISTS (SELECT 1 FROM maestro_mee WHERE codigo='FR-VID-GOTERO-10')",
        "INSERT INTO maestro_mee (codigo, descripcion, categoria, stock_actual, estado, fecha_creacion) SELECT 'FR-VID-OPALIZADO-50', 'FRASCO VIDRIO OPALIZADO 50ml', 'Frasco', 0, 'Activo', '2026-06-28 00:00:00' WHERE NOT EXISTS (SELECT 1 FROM maestro_mee WHERE codigo='FR-VID-OPALIZADO-50')",
        "UPDATE formula_headers SET activo=0 WHERE UPPER(producto_nombre) LIKE '%B3%BHA%'",
        "DELETE FROM producto_presentaciones WHERE (UPPER(producto_nombre) LIKE '%COLORETE%' OR UPPER(producto_nombre) LIKE '%BLUSH%') AND COALESCE(envase_codigo,'')=''",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V6', '6ml', 6, 'FR-AL-BLUSH-6', 1, 1 FROM formula_headers fh WHERE (UPPER(fh.producto_nombre) LIKE '%COLORETE%' OR UPPER(fh.producto_nombre) LIKE '%BLUSH%') AND COALESCE(fh.activo,1)=1 AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre AND pp.presentacion_codigo='V6')",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V10', '10ml', 10, 'FR-VID-GOTERO-10', 1, 1 FROM formula_headers fh WHERE (UPPER(fh.producto_nombre) LIKE '%CONTORNO%CAFEIN%') AND COALESCE(fh.activo,1)=1 AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre AND pp.presentacion_codigo='V10')",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V10', '10ml', 10, 'FR-VID-GOTERO-10', 1, 1 FROM formula_headers fh WHERE (UPPER(fh.producto_nombre) LIKE '%HIALURON%') AND COALESCE(fh.activo,1)=1 AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre AND pp.presentacion_codigo='V10')",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'V50', '50ml', 50, 'FR-VID-OPALIZADO-50', 1, 1 FROM formula_headers fh WHERE (UPPER(fh.producto_nombre) LIKE '%CENTELLA%') AND COALESCE(fh.activo,1)=1 AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre AND pp.presentacion_codigo='V50')",
    ]),
    (308, "Envases - backfill volumen_ml en producto_presentaciones desde sku_producto_map (Sebastian 28-jun: guardo muchos sin ml). Solo productos de UN volumen (los de 15+30 se resuelven en el tool por slot). Asi el plan calcula bien la demanda de envases.", [
        "UPDATE producto_presentaciones SET volumen_ml = (SELECT MIN(spm.volumen_ml) FROM sku_producto_map spm WHERE UPPER(TRIM(spm.producto_nombre))=UPPER(TRIM(producto_presentaciones.producto_nombre)) AND COALESCE(spm.volumen_ml,0)>0) WHERE COALESCE(volumen_ml,0)=0 AND COALESCE(envase_codigo,'')<>'' AND (SELECT COUNT(DISTINCT spm.volumen_ml) FROM sku_producto_map spm WHERE UPPER(TRIM(spm.producto_nombre))=UPPER(TRIM(producto_presentaciones.producto_nombre)) AND COALESCE(spm.volumen_ml,0)>0)=1",
    ]),
    (307, "Envases - auto-mapear producto->envase en producto_presentaciones (Sebastian 28-jun: limpiadores->redondo 150ml; Niacinamida/Multipeptidos/TRX->frasco con nombre 30ml). Asi el plan JALA envases. Idempotente (NOT EXISTS). El resto se mapea con el tool.", [
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'AUTO-150', '150ml', 150, (SELECT codigo FROM maestro_mee WHERE UPPER(descripcion) LIKE '%REDONDO%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE LOWER(fh.producto_nombre) LIKE '%limpiad%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE UPPER(descripcion) LIKE '%REDONDO%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo') AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre)",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'AUTO-30', '30ml', 30, (SELECT codigo FROM maestro_mee WHERE UPPER(descripcion) LIKE '%NIACINAMIDA%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE LOWER(fh.producto_nombre) LIKE '%niacinamida%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE UPPER(descripcion) LIKE '%NIACINAMIDA%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo') AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre)",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'AUTO-30', '30ml', 30, (SELECT codigo FROM maestro_mee WHERE UPPER(descripcion) LIKE '%MULTIPEPTIDOS%' AND UPPER(descripcion) NOT LIKE '%CONTORNO%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE LOWER(fh.producto_nombre) LIKE '%multipep%' AND LOWER(fh.producto_nombre) NOT LIKE '%contorno%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE UPPER(descripcion) LIKE '%MULTIPEPTIDOS%' AND UPPER(descripcion) NOT LIKE '%CONTORNO%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo') AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre)",
        "INSERT INTO producto_presentaciones (producto_nombre, presentacion_codigo, etiqueta, volumen_ml, envase_codigo, es_default, activo) SELECT fh.producto_nombre, 'AUTO-30', '30ml', 30, (SELECT codigo FROM maestro_mee WHERE UPPER(descripcion) LIKE '%TRX%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo' LIMIT 1), 1, 1 FROM formula_headers fh WHERE LOWER(fh.producto_nombre) LIKE '%trx%' AND COALESCE(fh.activo,1)=1 AND EXISTS (SELECT 1 FROM maestro_mee WHERE UPPER(descripcion) LIKE '%TRX%' AND codigo LIKE 'FR-%' AND COALESCE(estado,'Activo')='Activo') AND NOT EXISTS (SELECT 1 FROM producto_presentaciones pp WHERE pp.producto_nombre=fh.producto_nombre)",
    ]),
    ...
    (305, "Envases · cargar el inventario REAL del Excel como SALDO INICIAL (Sebastian 28-jun · columna TOTAL). 1 movimiento Entrada VIGENTE por envase (lote SALDO-EXCEL · idempotente NOT EXISTS · sin fecha=usa default PG-safe) matcheado por descripcion -> se MUESTRA en bodega. Recalcula stock_actual. ~66k uds.", [
        ...
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 3483, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('PLASTIC BOTTLE NO PRINT 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('PLASTIC BOTTLE NO PRINT 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 2046, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('SUERO MULTIPEPTIDOS 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('SUERO MULTIPEPTIDOS 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 263, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO AIRLESS CONTORNO 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO AIRLESS CONTORNO 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 3998, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO AZUL 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO AZUL 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 7510, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO VIDRIO OPALIZADO ENANO 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO VIDRIO OPALIZADO ENANO 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 510, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO BOOSTER TENSOR 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO BOOSTER TENSOR 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 8867, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO BLANCO CUADRADO 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO BLANCO CUADRADO 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 1, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO BLANCO CUADRADO 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO BLANCO CUADRADO 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 0, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('ENVASE METALICO MAXLASH 7,5ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('ENVASE METALICO MAXLASH 7,5ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 5021, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO PLASTIC INNER CUP  (interior del trx, pha) 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO PLASTIC INNER CUP  (interior del trx, pha) 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 891, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIP GLOSS BLANCO SIN SERG 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIP GLOSS BLANCO SIN SERG 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 271, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS CAFÉ OSCURO SERIG MOCA 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS CAFÉ OSCURO SERIG MOCA 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 766, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS  CAFÉ CLARO SIN SERIG 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS  CAFÉ CLARO SIN SERIG 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 384, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS PEACH 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS PEACH 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 780, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS MERLOT SIN SERG 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS MERLOT SIN SERG 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 762, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS MALVA SIN SERG 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS MALVA SIN SERG 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 1167, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO BLANCO PUFF 50ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO BLANCO PUFF 50ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 1734, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('FRASCO AIRLESS JAR BLANCO 50ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('FRASCO AIRLESS JAR BLANCO 50ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 5377, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('ENVASE REDONDO BLANCO 150ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('ENVASE REDONDO BLANCO 150ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 500, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS ROSADO CLARO SERIG sin lote BARBIE 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS ROSADO CLARO SERIG sin lote BARBIE 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 430, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS SALMON SERIG sin lote 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS SALMON SERIG sin lote 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 634, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('LIPS GLOSS GRIS SERIG sin lote 10ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('LIPS GLOSS GRIS SERIG sin lote 10ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 234, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('ENVASE CONTORNO RETINALDEHIDO 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('ENVASE CONTORNO RETINALDEHIDO 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 997, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Paleta rubores x6 negra -')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Paleta rubores x6 negra -')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 937, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Frasco cilíndrico blanco logo -')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Frasco cilíndrico blanco logo -')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 990, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Frasco rubor en povlo de vidrio con tapa negra')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Frasco rubor en povlo de vidrio con tapa negra')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 2919, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Envase conceler tube')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Envase conceler tube')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 0, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('ENVASE CONTORNO MULTIPEPTIDOS 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('ENVASE CONTORNO MULTIPEPTIDOS 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 845, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Tampografia · Vitamina C+ 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Tampografia · Vitamina C+ 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 700, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Tampografia · AZHYBRID  30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Tampografia · AZHYBRID  30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 353, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Tampografia · AZHYBRID  15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Tampografia · AZHYBRID  15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 92, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Tampografia · Renova C10 30ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Tampografia · Renova C10 30ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 462, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Tampografia · Vitamina C+ 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Tampografia · Vitamina C+ 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 353, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Tampografia · Triactive 15ml')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Tampografia · Triactive 15ml')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 808, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · Gel Hidratante')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · Gel Hidratante')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 626, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · Limpiador kojico')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · Limpiador kojico')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 177, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · Limpiador BHA 2%')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · Limpiador BHA 2%')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 82, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · Emulsion limpiadora')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · Emulsion limpiadora')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 238, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · Nova pha')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · Nova pha')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 126, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · Crema urea facial KELLY')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · Crema urea facial KELLY')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 300, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('Con Serigrafia · CREMA CORPORAL DE UREA  KELLY GUERRA')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('Con Serigrafia · CREMA CORPORAL DE UREA  KELLY GUERRA')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 338, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM MALVA')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM MALVA')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 474, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM HOT PINK')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM HOT PINK')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 258, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM CAROLINA')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM CAROLINA')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 416, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM PEACH')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM PEACH')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 359, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM HIMALAYAN PINK')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM HIMALAYAN PINK')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 420, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM BORGOÑA')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM BORGOÑA')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 306, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM CINNAMON')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM CINNAMON')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 532, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('MAXLASH')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('MAXLASH')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 732, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BLUSH BALM')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BLUSH BALM')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 988, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('SUERO MULTIPEPTIDOS')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('SUERO MULTIPEPTIDOS')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 982, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('SUERO ACIDO HIDRATANTE AH')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('SUERO ACIDO HIDRATANTE AH')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 932, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('SUERO RENOVA C10')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('SUERO RENOVA C10')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 1128, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('SUERO ILUMINADOR TRX')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('SUERO ILUMINADOR TRX')) AND mv2.lote_ref='SALDO-EXCEL')",
        "INSERT INTO movimientos_mee (mee_codigo, tipo, cantidad, unidad, lote_ref, responsable, observaciones, estado) SELECT codigo, 'Entrada', 1375, 'und', 'SALDO-EXCEL', 'inventario', 'Saldo inicial Excel 28-jun', 'VIGENTE' FROM maestro_mee WHERE UPPER(TRIM(descripcion))=UPPER(TRIM('BOOSTER')) AND COALESCE(estado,'Activo')='Activo' AND NOT EXISTS (SELECT 1 FROM movimientos_mee mv2 JOIN maestro_mee mm2 ON mv2.mee_codigo=mm2.codigo WHERE UPPER(TRIM(mm2.descripcion))=UPPER(TRIM('BOOSTER')) AND mv2.lote_ref='SALDO-EXCEL')",
        "UPDATE maestro_mee SET stock_actual=(SELECT COALESCE(SUM(CASE WHEN tipo IN ('Entrada','Ajuste') THEN cantidad WHEN tipo='Salida' THEN -cantidad ELSE 0 END),0) FROM movimientos_mee mv WHERE mv.mee_codigo=maestro_mee.codigo AND COALESCE(mv.anulado,0)=0) WHERE codigo IN (SELECT DISTINCT mee_codigo FROM movimientos_mee WHERE lote_ref='SALDO-EXCEL')",
    ]),
...
('FR-MULP-30','SUERO MULTIPEPTIDOS 30ml','Frasco',2046,'Activo','2026-06-27'),
('FR-AIRLESSCON-15','FRASCO AIRLESS CONTORNO 15ml','Frasco',263,'Activo','2026-06-27'),
('FR-AZUL-30','FRASCO AZUL 30ml','Frasco',3998,'Activo','2026-06-27'),
('FR-VIDRIOOPAL-30','FRASCO VIDRIO OPALIZADO ENANO 30ml','Frasco',7510,'Activo','2026-06-27'),
('FR-BOOSTERTEN-15','FRASCO BOOSTER TENSOR 15ml','Frasco',510,'Activo','2026-06-27'),
('FR-BLANCOCUAD-30','FRASCO BLANCO CUADRADO 30ml','Frasco',8867,'Activo','2026-06-27'),
('FR-BLANCOCUAD-15','FRASCO BLANCO CUADRADO 15ml','Frasco',1,'Activo','2026-06-27'),
('FR-METALICOMA-7.5','ENVASE METALICO MAXLASH 7,5ml','Frasco',0,'Activo','2026-06-27'),
('FR-PLASTICINN-30','FRASCO PLASTIC INNER CUP  (interior del trx, pha) 30ml','Frasco',5021,'Activo','2026-06-27'),
('FR-GLOSS-10-BLANCO','LIP GLOSS BLANCO SIN SERG 10ml','Frasco',891,'Activo','2026-06-27'),
('FR-GLOSS-10-MOCA','LIPS GLOSS CAFÉ OSCURO SERIG MOCA 10ml','Frasco',442,'Activo','2026-06-27'),
('FR-GLOSS-10-CAFECLARO','LIPS GLOSS  CAFÉ CLARO SIN SERIG 10ml','Frasco',896,'Activo','2026-06-27'),
('FR-GLOSS-10-PEACH','LIPS GLOSS PEACH 10ml','Frasco',674,'Activo','2026-06-27'),
('FR-GLOSS-10-MERLOT','LIPS GLOSS MERLOT SIN SERG 10ml','Frasco',780,'Activo','2026-06-27'),
('FR-GLOSS-10-MALVA','LIPS GLOSS MALVA SIN SERG 10ml','Frasco',962,'Activo','2026-06-27'),
('FR-BLANCOPUFF-50','FRASCO BLANCO PUFF 50ml','Frasco',1167,'Activo','2026-06-27'),
('FR-AIRLESSJAR-50','FRASCO AIRLESS JAR BLANCO 50ml','Frasco',1734,'Activo','2026-06-27'),
('FR-REDONDOBLA-150','ENVASE REDONDO BLANCO 150ml','Frasco',5377,'Activo','2026-06-27'),
('FR-GLOSS-10-BARBIE','LIPS GLOSS ROSADO CLARO SERIG sin lote BARBIE 10ml','Frasco',500,'Activo','2026-06-27'),
('FR-GLOSS-10-SALMON','LIPS GLOSS SALMON SERIG sin lote 10ml','Frasco',430,'Activo','2026-06-27'),
('FR-GLOSS-10-GRIS','LIPS GLOSS GRIS SERIG sin lote 10ml','Frasco',634,'Activo','2026-06-27'),
('FR-CONTORNORE-15','ENVASE CONTORNO RETINALDEHIDO 15ml','Frasco',234,'Activo','2026-06-27'),
('FR-PALETARUBO','Paleta rubores x6 negra -','Frasco',1757,'Activo','2026-06-27'),
('FR-CILINDRICO','Frasco cilíndrico blanco logo -','Frasco',937,'Activo','2026-06-27'),
('FR-RUBORENPOV','Frasco rubor en povlo de vidrio con tapa negra','Frasco',990,'Activo','2026-06-27'),
('FR-CONCELERTU','Envase conceler tube','Frasco',2919,'Activo','2026-06-27'),
('FR-CONTORNOMU-15','ENVASE CONTORNO MULTIPEPTIDOS 15ml','Frasco',0,'Activo','2026-06-27'),
('IMP-TP-VITC-30','Con Tampografia · Vitamina C+ 30ml','Impresión',881,'Activo','2026-06-27'),
('IMP-TP-AZH-30','Con Tampografia · AZHYBRID  30ml','Impresión',700,'Activo','2026-06-27'),
('IMP-TP-AZH-15','Con Tampografia · AZHYBRID  15ml','Impresión',353,'Activo','2026-06-27'),
('IMP-TP-RECN-30','Con Tampografia · Renova C10 30ml','Impresión',92,'Activo','2026-06-27'),
('IMP-TP-VITC-15','Con Tampografia · Vitamina C+ 15ml','Impresión',462,'Activo','2026-06-27'),
('IMP-TP-TRIAC-15','Con Tampografia · Triactive 15ml','Impresión',0,'Activo','2026-06-27'),
('IMP-SG-GELH','Con Serigrafia · Gel Hidratante','Impresión',808,'Activo','2026-06-27'),
('IMP-SG-LKJ','Con Serigrafia · Limpiador kojico','Impresión',626,'Activo','2026-06-27'),
('IMP-SG-LBHA-2','Con Serigrafia · Limpiador BHA 2%','Impresión',177,'Activo','2026-06-27'),
('IMP-SG-EMLIM','Con Serigrafia · Emulsion limpiadora','Impresión',82,'Activo','2026-06-27'),
('IMP-SG-NPHA','Con Serigrafia · Nova pha','Impresión',238,'Activo','2026-06-27'),
('IMP-SG-CFUREA','Con Serigrafia · Crema urea facial KELLY','Impresión',126,'Activo','2026-06-27'),
('IMP-SG-CCUREA','Con Serigrafia · CREMA CORPORAL DE UREA  KELLY GUERRA','Impresión',300,'Activo','2026-06-27'),
('ETQ-BLUSH-MALVA','BLUSH BALM MALVA','Etiqueta',338,'Activo','2026-06-27'),
('ETQ-BLUSH-HOTPINK','BLUSH BALM HOT PINK','Etiqueta',474,'Activo','2026-06-27'),
('ETQ-BLUSH-CAROLINA','BLUSH BALM CAROLINA','Etiqueta',258,'Activo','2026-06-27'),
('ETQ-BLUSH-PEACH','BLUSH BALM PEACH','Etiqueta',416,'Activo','2026-06-27'),
('ETQ-BLUSH-HIMALAYANPINK','BLUSH BALM HIMALAYAN PINK','Etiqueta',359,'Activo','2026-06-27'),
('ETQ-BLUSH-BORGONA','BLUSH BALM BORGOÑA','Etiqueta',420,'Activo','2026-06-27'),
('ETQ-BLUSH-CINNAMON','BLUSH BALM CINNAMON','Etiqueta',306,'Activo','2026-06-27'),
('CJA-MAXLASH','MAXLASH','Plegadiza',532,'Activo','2026-06-27'),
('CJA-BLUSH','BLUSH BALM','Plegadiza',732,'Activo','2026-06-27'),
('CJA-MULP','SUERO MULTIPEPTIDOS','Plegadiza',988,'Activo','2026-06-27'),
