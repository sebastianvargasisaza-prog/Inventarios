...
async function loadAgencia(force=false) {
  ...
  document.getElementById('ag-proposals').innerHTML = '<div style="color:#64748b;font-size:13px;">Generando propuestas...</div>';
  try {
    const r = await fetch('/api/marketing/agencia/audit');
    const d = await r.json();
...
function sevLabel(sev) {
  return {critical:'CRÍTICO',high:'ALTO',medium:'MEDIO',low:'BAJO'}[sev]||sev.toUpperCase();
}
...
function renderAgencia(d) {
  ...
  // KPIs
  document.getElementById('ag-kpi-activos').textContent = portfolio.activos || 0;
  const avgScore = influencers.length ? Math.round(influencers.reduce((s,i)=>s+(i.score||0),0)/influencers.length) : 0;
  document.getElementById('ag-kpi-score').textContent = avgScore + '/100';
  document.getElementById('ag-kpi-score').style.color = scoreColor(avgScore);
  document.getElementById('ag-kpi-riesgo').textContent = portfolio.en_riesgo || 0;
  const criticals = audit.filter(a=>a.severity==='critical').length;
  document.getElementById('ag-kpi-critical').textContent = criticals;
  const totalInv = influencers.reduce((s,i)=>s+(i.total_pagado||0),0);
  document.getElementById('ag-kpi-inversion').textContent = fmtM(totalInv);

  // Scoring table
  const sorted = [...influencers].sort((a,b)=>(b.score||0)-(a.score||0));
  let rows = '';
  sorted.forEach(inf => {
    const est = inf.estado === 'Activo'
      ? '<span style="color:#34d399;font-size:11px;font-weight:700;">Activo</span>'
      : '<span style="color:#64748b;font-size:11px;">Inactivo</span>';
    rows += `<tr style="border-bottom:1px solid #1e293b;">
      <td style="padding:8px;font-weight:600;color:#e2e8f0;">${inf.nombre||'—'}</td>
      <td style="padding:8px;color:#94a3b8;font-size:12px;">${inf.nicho||'—'}</td>
      <td style="padding:8px;text-align:center;">${scoreBadge(inf.score||0)}</td>
      <td style="padding:8px;text-align:right;color:#94a3b8;">${inf.engagement_rate ? (inf.engagement_rate*100).toFixed(1)+'%' : '—'}</td>
      <td style="padding:8px;text-align:right;color:#94a3b8;">${inf.seguidores ? Number(inf.seguidores).toLocaleString('es-CO') : '—'}</td>
      <td style="padding:8px;text-align:right;color:#94a3b8;">${inf.campanas_count||0}</td>
      <td style="padding:8px;text-align:right;color:#34d399;font-weight:600;">${fmtM(inf.total_pagado||0)}</td>
      <td style="padding:8px;text-align:center;">${est}</td>
    </tr>`;
  });
  document.getElementById('ag-scoring-tbody').innerHTML = rows || '<tr class="empty-row"><td colspan="8">Sin influencers registrados.</td></tr>';

  // Audit
  const bySev = {critical:[],high:[],medium:[],low:[]};
  audit.forEach(a => { if(bySev[a.severity]) bySev[a.severity].push(a); });
  let auditHtml = '';
  ...
  document.getElementById('ag-audit-list').innerHTML = auditHtml || '<div style="color:#34d399;font-size:13px;">&#x2705; Sin hallazgos relevantes.</div>';

  // Competition
  const niches = competition.niches || {};
  const gaps = competition.gaps || [];
  let compHtml = '';
  if (Object.keys(niches).length) {
    compHtml += `<div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;">Distribución por nicho</div>`;
    const total = Object.values(niches).reduce((s,v)=>s+v,0);
    const sorted_niches = Object.entries(niches).sort((a,b)=>b[1]-a[1]);
    sorted_niches.forEach(([nicho, cnt]) => {
      const pct = total ? Math.round(cnt/total*100) : 0;
      compHtml += `<div style="margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;font-size:12px;color:#94a3b8;margin-bottom:3px;">
          <span>${nicho}</span><span>${cnt} · ${pct}%</span>
        </div>
        <div style="background:#1e293b;border-radius:4px;height:6px;">
  ...
  document.getElementById('ag-competition').innerHTML = compHtml || '<div style="color:#64748b;font-size:13px;">Sin datos suficientes para análisis competitivo.</div>';

  // Proposals
  let propHtml = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;">';
  if (proposals.length) {
    proposals.forEach(p => {
      const priCol = p.priority === 'alta' ? '#f87171' : (p.priority === 'media' ? '#f59e0b' : '#34d399');
      propHtml += `<div style="background:#1e293b;border:1px solid #334155;border-radius:12px;padding:16px;border-left:3px solid ${priCol};">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
          <div style="font-weight:700;color:#e2e8f0;font-size:14px;">${p.title}</div>
          <span style="font-size:10px;font-weight:700;color:${priCol};text-transform:uppercase;background:${priCol}22;padding:2px 8px;border-radius:10px;">${p.priority||'media'}</span>
        </div>
        <div style="color:#94a3b8;font-size:12px;line-height:1.5;margin-bottom:10px;">${p.description}</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ${p.budget_est ? `<span style="font-size:11px;color:#34d399;background:#34d39922;padding:2px 8px;border-radius:8px;">&#x1F4B0; ${p.budget_est}</span>` : ''}
    ...
    propHtml += '<div style="color:#64748b;font-size:13px;padding:20px;">Sin propuestas generadas. Registra más datos de campañas para activar recomendaciones.</div>';
  }
  propHtml += '</div>';
  document.getElementById('ag-proposals').innerHTML = propHtml;
}
